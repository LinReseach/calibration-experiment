...
def calibration(win, fileName, calibration, pc='default', nrPoints=15, dotColor=[255, 255, 255], ip="192.168.0.167", port= 12345, camera = 4):
    ...
    if nrPoints == 9:
        ...
        xlineLength = (xSize - xSize / 11) / 4
        yLineLength = (ySize - ySize / 11  ) / 2
    ...
    time.sleep(1) #why I need this ,maybe because of thread protocal,look at server.py or ask people
    ...
    startKey = \
        drawText(win, textSize=xSize / 40, text='Training! \n 24 dots will appear on the screen one '
                                                'after the other in random order \n per each position', textKey=['space', 'escape'])[0]
    ...
    drawText(win, textSize=xSize / 30,
             text='During the training you will test the \n experiment for the first two position. \n\n Press space to start the training ',
             textKey=['space', 'escape'])[0]
    ...
    for i in range(0, len(gridPoints)):
        ...
        if (i + 1) % (nrPoints * 2) == 0 and i - 1 != len(gridPoints):
            ...
            time.sleep(0.5)
    drawText(win, textSize=xSize / 30,
             text="Thanks for your time:) !!  The experiment is finished \n You are free to take a break and to ask any question")
    win.flip()
...
if __name__ == '__main__':
    ...
    #print('mon', mon)
    #exit()
    #print('size', monitors.Monitor(mon[0]).getSizePix())
    #exit()
    
    win = visual.Window(pos=(0, 0), units='pix', monitor=mon, size=screenRes,
                        colorSpace='rgb255',
