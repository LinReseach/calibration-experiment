...
def calibration(win, fileName, calibration, calibration4k, pc='default', nrPoints=15, dotColor=[255, 255, 255], ip="192.168.0.167", port= 12345, camera = 4):
    """
    ...
    # wrong arrow robot reaction sentences
    dict = {0: 'I will not store these pictures' , 1: 'ops, try to do better', 2: 'you miss it!', 3: 'You are loosing focus',
            4: 'Hey, you just need to look a dot', 5: "I know... It's a boring task", 6: "Soon I will improve my gazing skills",
            7: "At the end of the section you can take a break", 8: "The better you are, the better I will be", 9: "Maybe we selected the wrong participant"}
    ...
    startKey = \
    text = "This is the training, 30 dots will appear on the screen one after the other in random order"
    connect.say(text)
    ...
    time.sleep(0.5)
    text = "You must look each dot and with the keyboard select the correct arrow direction"
    connect.say(text)
    drawText(win, textSize=xSize / 40, text=' You must look each dot and with the keyboard select the correct arrow direction [space]',
    ...
    for i in range(0, len(gridPoints)):
        ...
        #     text = 'break! Go to position: ', str(pos), ', Press space to continue'
        #     connect.say(text)
        #     drawText(win, textSize=xSize / 30,
    ...
    for i in range(0, len(gridPoints)):
        ...
        while True:
            ...
            if (time.time() - s) > waitForSacc:
                ...
                calibration4k = '4k'+calibration
                cv2.imwrite(calibration4k + '\\' + fName, img4k)
        ...
        if (i + 1) % (nrPoints * 2) == 0 and i - 1 != len(gridPoints):
            ...
            win.flip()
            time.sleep(0.5)
    text = 'Thanks for your attention, the experiment is ended. Take a break and ask any question you want'
    connect.say(text)
    time.sleep(0.10)
    drawText(win, textSize=xSize / 30,
...
def getFileName2():
    ...
    participant4k = '4k' + participant
    # Check for existing files
    if not os.path.exists(participant):
        os.mkdir(participant)
        os.mkdir(participant4k)
    else:
        it = 1
        newDir = participant
        while os.path.exists(newDir):
...
if __name__ == '__main__':
    ...
    participant4k = '4k'+participant
    dataframe.to_pickle(participant4k + '\\' + fileName + 'Header.p')
    dataframe.to_csv(participant4k + '\\' + fileName + 'Header.csv', index=False)
